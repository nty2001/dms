			...
			if($qry){
				...
				if(!empty($_FILES['img']['tmp_name'])){
					if(!is_dir(base_app."uploads/individual"))
						mkdir(base_app."uploads/individual");
					$ext = pathinfo($_FILES['img']['name'], PATHINFO_EXTENSION);
					...
					$upload =imagepng($temp,base_app.$fname);
					if($upload){
						$this->conn->query("UPDATE `individual_list` set `avatar` = CONCAT('{$fname}', '?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '{$id}'");
						if($this->settings->userdata('id') == $id)
						$this->settings->set_userdata('avatar',$fname."?v=".time());
					}
	...
	function registration(){
		...
		if($save){
			...
			if(!empty($_FILES['img']['tmp_name'])){
				...
				$upload =imagepng($temp,base_app.$fname);
				if($upload){
					$this->conn->query("UPDATE `individual_list` set `avatar` = CONCAT('{$fname}', '?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '{$uid}'");
				}
				imagedestroy($temp);
			}
		}else{
